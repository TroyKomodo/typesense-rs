...
pub struct SearchOverrideRule {
	/// Indicates that the override should apply when the filter_by parameter in
	/// a search query exactly matches the string specified here (including
	/// backticks, spaces, brackets, etc).
	#[serde(rename = "filter_by", skip_serializing_if = "Option::is_none")]
	pub filter_by: Option<String>,
	/// Indicates whether the match on the query term should be `exact` or
...
/// Indicates whether the match on the query term should be `exact` or
/// `contains`. If we want to match all queries that contained the word `apple`,
/// we will use the `contains` match instead.
