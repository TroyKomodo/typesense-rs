...
pub struct Field {
	...
	/// Enables an index optimized for range filtering on numerical fields (e.g.
	/// rating:>3.5). Default: false.
	#[serde(rename = "range_index", skip_serializing_if = "Option::is_none")]
	pub range_index: Option<bool>,
	/// Name of a field in another collection that should be linked to this
	/// collection so that it can be joined during query.
	...
	/// When set to false, the field value will not be stored on disk. Default:
	/// true.
	#[serde(rename = "store", skip_serializing_if = "Option::is_none")]
	pub store: Option<bool>,
	#[serde(rename = "type")]
	pub r#type: String,
	/// The distance metric to be used for vector search. Default: `cosine`. You
	/// can also use `ip` for inner product.
